...
class CarEnv(gym.Env):
    ...
    def check_collision(self, car_model, obstacles, margin=0,
        ...
        :param margin: margin of error for collision - if the distance is smaller than the margin - the car colided
    ...
    def step(self, action):
        ...
        if self.check_collision(self.car_model, self.bodies):
            self.crashed = True
        if scan_to_map.dist(self.pos, self.end_point) < consts.min_dist_to_target:
            self.finished = True
        ...
        # saving for later
        self.swivel = self.steeringAngle # TODO: change to getJointState
